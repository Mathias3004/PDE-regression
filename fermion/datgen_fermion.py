...
from CustomFermiHubbardModel import *
...
def observables(eng, data):
    keys = ['t', 'entropy', 'N', 'g1', 'g2', 'trunc_err']
    if data is None:
    ...
    data['g1'].append(eng.psi.correlation_function('Cd', 'C'))
...
if __name__ == "__main__":
    ...
    # fixed parameters
    t = 0.5 # tunneling
    U2 = 0. # next nearest-neighbor interaction
    ...
    model_params = dict(L=N_site, t=t, U=U, U2=U2, mu=mu, bc_MPS=bc_MPS, conserve="N", n_max=n_max)
    FH0 = CustomFermionChain(model_params)

    FH0.add_onsite(potential, 0, "N", category=None)
    FH0.init_H_from_terms()
    
    # initial state
    occ = np.zeros(N_site, dtype="int")
    occ[:N_particle] = 1
    occ = np.roll(occ,(N_site-N_particle)//2 )
    psi = MPS.from_product_state(FH0.lat.mps_sites(), occ, bc=FH0.lat.bc_MPS)
    
    # TEBD engine

    print("Start DMRG...")
    start = time.time()
    eng = dmrg.TwoSiteDMRGEngine(psi, FH0, dmrg_params)
    E, psi = eng.run()
    ...
    model_params = dict(L=N_site, t=t, U=U, U2=U2, mu=mu, bc_MPS=bc_MPS, conserve="N", n_max=n_max)
    FH = CustomFermionChain(model_params)
    
    # TEBD engine with DMRG obtained psi
    eng = tebd.TEBDEngine(psi, FH, tebd_params)
    ...
    while eng.evolved_time < t_ev:
        ...
        if save:
          ...
          suffix = "N_" + str(N_site) + "_V0_" + "{:.2f}".format(V0) + "_U_" + "{:.2f}".format(U) + "_U2_" + "{:.2f}".format(U2) + "_t_" + "{:.2f}".format(t) + "_T_" + "{:.2f}".format(t_ev)
